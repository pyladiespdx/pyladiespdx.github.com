...
For example, a list comprehension ```[item for item in items]``` 

is up to twice as fast as the equivalent ```for item in items: append(item)```   
...
Search __patterns__ are often appended by an "r":
